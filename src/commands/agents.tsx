...
function generateAgentFileContent(
  ...
  const lines = ['---', `name: ${agentType}`, `description: ${formattedDescription}`]
...
function MultilineTextInput({
  ...
  return (
    <Box flexDirection="column" width="100%">
      ...
      <Box flexDirection="column">
        ...
        >
          <Box flexDirection="column">
            ...
            {focus && cursorBlink && hasContent && <Text color={theme.primary}>_</Text>}
          </Box>
        </Box>
        ...
        <Box marginTop={1} flexDirection="row" justifyContent="space-between">
          ...
          <Box>
            {error ? (
              ...
              <Text dimColor>{hasContent ? 'Ready' : 'Waiting'}</Text>
            )}
          </Box>
        </Box>
      </Box>
      ...
      {/* Instructions */}
      <Box marginTop={1}>
        <Text dimColor>Press Enter to submit · Shift+Enter for new line</Text>
      </Box>
    </Box>
  )
}
...
function AgentsUI({onExit}: AgentsUIProps) {
  ...
  switch (modeState.mode) {
    ...
    case 'create-location':
      return <LocationSelect createState={createState} setCreateState={setCreateState} setModeState={setModeState} />

    case 'create-method':
      return <MethodSelect createState={createState} setCreateState={setCreateState} setModeState={setModeState} />
    ...
    case 'create-description':
      return <DescriptionStep createState={createState} setCreateState={setCreateState} setModeState={setModeState} />
    ...
    case 'create-model':
      return <ModelStep createState={createState} setCreateState={setCreateState} setModeState={setModeState} />

    case 'create-color':
      return <ColorStep createState={createState} setCreateState={setCreateState} setModeState={setModeState} />

    case 'create-prompt':
      return <PromptStep createState={createState} setCreateState={setCreateState} setModeState={setModeState} />
    ...
    case 'agent-menu':
      return <AgentMenu agent={modeState.selectedAgent!} setModeState={setModeState} />

    case 'view-agent':
      return <ViewAgent agent={modeState.selectedAgent!} tools={tools} setModeState={setModeState} />

    case 'edit-agent':
      return <EditMenu agent={modeState.selectedAgent!} setModeState={setModeState} />
...
function AgentListView({location, agents, allAgents, onBack, onSelect, onCreateNew, changes}: AgentListProps) {
  const theme = getTheme()
  ...
  const renderCreateOption = () => (
    <Box flexDirection="row" gap={1}>
      <Text color={onCreateOption ? theme.primary : undefined}>{onCreateOption ? `${UI_ICONS.pointer} ` : '  '}</Text>
  ...
  const displayAgents = useMemo(() => {
    if (currentLocation === 'all') {
      return [...customAgents.filter(a => a.location === 'user'), ...customAgents.filter(a => a.location === 'project')]
    ...
    return (
      <Box flexDirection="column">
        ...
        <Header title="🤖 Agents" subtitle="">
          {onCreateNew && <Box marginY={1}>{renderCreateOption()}</Box>}
          <Box marginTop={1} flexDirection="column">
            ...
            <Text>
              Specialized AI assistants that Kode can delegate to for specific tasks, compatible with Claude Code
              `.claude` agent packs.
            </Text>
    ...
    <Box flexDirection="column">
      <Header title="🤖 Agents" subtitle="">
        ...
        <Box flexDirection="column" marginTop={1}>
          {onCreateNew && <Box marginBottom={1}>{renderCreateOption()}</Box>}
      ...
      <InstructionBar
        instructions={
          inLocationTabs ? '←→ Switch tabs • Enter Select • Tab Exit tabs' : '↑↓ Navigate • Tab Location • Enter Select'
        }
      />
    </Box>
  )
...
function ToolsStep({createState, setCreateState, setModeState, tools}: ToolsStepProps) {
  ...
  const initialSelection =
    createState.selectedTools.length > 0 ? new Set(createState.selectedTools) : new Set(tools.map(t => t.name)) // Select all tools by default
    ...
    <Box flexDirection="column">
      <Header title="🔧 Tool Permissions" subtitle="" step={3} totalSteps={5}>
        <Box flexDirection="column" marginTop={1}>
          ...
          <Box marginTop={1}>
            <Text dimColor>
              {allSelected ? 'All tools selected' : `${selectedTools.size} of ${tools.length} tools selected`}
            </Text>
            {selectedCategory !== 'all' && <Text dimColor>Filtering: {selectedCategory} tools</Text>}
          </Box>
        </Box>
      </Header>
          ...
          {modelOptions.map((model, index) => {
            ...
            return (
              <Box key={model.id || 'inherit'} marginBottom={0}>
                <Box flexDirection="row" gap={1}>
                  <Text color={isSelected ? theme.primary : undefined}>{isSelected ? UI_ICONS.pointer : ' '}</Text>
          ...
          {colors.map((color, idx) => {
            ...
            return (
              <Box key={idx} flexDirection="row">
                <Text color={isSelected ? theme.primary : undefined}>{isSelected ? '❯ ' : '  '}</Text>
                <Box minWidth={12}>
      ...
      <Header title="✅ Review & Create" subtitle="">
        <Box flexDirection="column" marginTop={1}>
          ...
          <Box flexDirection="column" gap={0}>
            <Text>
              • <Text bold>Agent ID:</Text> {createState.agentType}
            </Text>
            <Text>
              • <Text bold>Location:</Text> {createState.location === 'project' ? 'Project' : 'Personal'}
            </Text>
            <Text>
              • <Text bold>Tools:</Text> {toolNames.length > 50 ? toolNames.slice(0, 50) + '...' : toolNames}
            </Text>
            <Text>
              • <Text bold>Model:</Text> {getDisplayModelName(createState.selectedModel)}
            </Text>
            {createState.selectedColor && (
              <Text>
                • <Text bold>Color:</Text> <Text color={createState.selectedColor}>{createState.selectedColor}</Text>
              </Text>
            )}
          </Box>
          ...
          <Box marginTop={2}>{isCreating ? <LoadingSpinner text="Creating agent..." /> : null}</Box>
        </Box>
      </Header>
...
function MethodSelect({createState, setCreateState, setModeState}: MethodSelectProps) {
  ...
  const handleChange = (value: string) => {
    setCreateState({
      type: 'SET_METHOD',
      value: value as 'generate' | 'manual'
    })
    if (value === 'generate') {
  ...
  return (
    <Box flexDirection="column">
      <Header title="Create new agent" subtitle="Creation method" step={2} totalSteps={9}>
        <Box marginTop={1}>
          <SelectList options={options} selectedIndex={selectedIndex} onChange={handleChange} onCancel={handleCancel} />
        </Box>
      </Header>
      <InstructionBar />
    </Box>
  ...
  return (
    <Box flexDirection="column">
      <Header title={`Edit agent: ${agent.agentType}`} subtitle={`Location: ${agent.location}`}>
        <Box marginTop={1}>
          <SelectList options={options} selectedIndex={selectedIndex} onChange={handleSelect} numbered={false} />
        </Box>
      </Header>
...
function EditToolsStep({agent, tools, setModeState, onAgentUpdated}: EditToolsStepProps) {
  ...
  const allSelected = selectedTools.size === tools.length && tools.length > 0
  const readSelected =
    categorizedTools.read.every(tool => selectedTools.has(tool.name)) && categorizedTools.read.length > 0
  const editSelected =
    categorizedTools.edit.every(tool => selectedTools.has(tool.name)) && categorizedTools.edit.length > 0
  const execSelected =
    categorizedTools.execution.every(tool => selectedTools.has(tool.name)) && categorizedTools.execution.length > 0
...
function EditModelStep({agent, setModeState, onAgentUpdated}: EditModelStepProps) {
  ...
  const modelOptions = [
    ...
    ...profiles.map((p: any) => ({
      id: p.modelName,
      name: p.name,
      description: `${p.provider || 'provider'} · ${p.modelName}`
    }))
  ]
  ...
  const handleSave = async (color: string | null) => {
    ...
    try {
      ...
      if (updatedAgent) {
        ...
        const fallbackAgent: AgentConfig = {
          ...agent,
          ...(colorValue ? {color: colorValue} : {color: undefined})
        }
  ...
  return (
    <Box flexDirection="column">
      <Header title={`Agent: ${agent.agentType}`} subtitle="Details">
        <Box flexDirection="column" marginTop={1}>
          <Text>
            <Text bold>Type:</Text> {agent.agentType}
          </Text>
          <Text>
            <Text bold>Location:</Text> {agent.location} {locationPath !== '(built-in)' ? `· ${locationPath}` : ''}
          </Text>
          <Text>
            <Text bold>Description:</Text> {agent.whenToUse}
          </Text>
          <Text>
            <Text bold>Model:</Text> {displayModel}
          </Text>
          <Text>
            <Text bold>Color:</Text> {agent.color || 'auto'}
          </Text>
...
function EditAgent({agent, tools, setModeState, onAgentUpdated}: EditAgentProps) {
      ...
      case 'confirm':
        ...
        return (
          <Box flexDirection="column">
            ...
            <Box flexDirection="column" marginTop={1}>
              <Text>
                <Text bold>Agent:</Text> {agent.agentType}
              </Text>
              <Text>
                <Text bold>Description:</Text> {editedDescription}
              </Text>
              <Text>
                <Text bold>Tools:</Text> {editedTools.includes('*') ? 'All tools' : editedTools.join(', ')}
              </Text>
              <Text>
                <Text bold>System Prompt:</Text> {editedPrompt.slice(0, 100)}
                {editedPrompt.length > 100 ? '...' : ''}
              </Text>
              ...
              <Box marginTop={2}>
                {isUpdating ? <LoadingSpinner text="Updating agent..." /> : <Text>Press Enter to save changes</Text>}
              </Box>
            </Box>
          </Box>
        )
  ...
  return (
    <Box flexDirection="column">
      ...
      >
        <Box marginTop={1}>{renderStepContent()}</Box>
      </Header>
      <InstructionBar
        instructions={
          currentStep === 'confirm' ? 'Press Enter to save · Esc to go back' : 'Enter to continue · Esc to go back'
        }
      />
    </Box>
  )
      ...
      <Header title="Delete agent" subtitle={`Delete "${agent.agentType}"?`}>
        <Box marginTop={1}>
          ...
          <Box marginTop={2} gap={3}>
            <Text color={!selected ? 'cyan' : undefined}>{!selected ? `${UI_ICONS.pointer} ` : '  '}No</Text>
            <Text color={selected ? 'red' : undefined}>{selected ? `${UI_ICONS.pointer} ` : '  '}Yes, delete</Text>
          </Box>
        </Box>
      </Header>
      <InstructionBar />
