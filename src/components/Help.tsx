...
import {getCustomCommandDirectories, hasCustomCommands, type CustomCommandWithScope} from '@services/customCommands'
 ...
 */
export function Help({commands, onClose}: {commands: Command[]; onClose: () => void}): React.ReactNode {
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Box marginTop={1} flexDirection="column">
        <Text>
          {PRODUCT_NAME} is a beta research preview. Always review {PRODUCT_NAME}&apos;s responses, especially when
          running code. {PRODUCT_NAME} has read access to files in the current directory and can run commands and edit
      ...
      {count >= 2 && (
        <Box marginTop={1} flexDirection="column">
          ...
          <Text>
            • Ask questions about your codebase <Text color={getTheme().secondaryText}>&gt; How does foo.py work?</Text>
          </Text>
          <Text>
            • Edit files <Text color={getTheme().secondaryText}>&gt; Update bar.ts to...</Text>
          </Text>
          <Text>
            • Fix errors <Text color={getTheme().secondaryText}>&gt; cargo build</Text>
          </Text>
          <Text>
            • Run commands <Text color={getTheme().secondaryText}>&gt; /help</Text>
          </Text>
          <Text>
            • Run bash commands <Text color={getTheme().secondaryText}>&gt; !ls</Text>
          </Text>
        </Box>
      )}
      ...
      {count >= 3 && (
        <Box marginTop={1} flexDirection="column">
                ...
                {customCommands.map((cmd, i) => (
                  <Box key={i} marginLeft={1}>
                    ...
                    {cmd.aliases && cmd.aliases.length > 0 && (
                      <Text color={theme.secondaryText}> (aliases: {cmd.aliases.join(', ')})</Text>
                    )}
                    {/* Show scope indicator for debugging */}
                    {cmd.scope && <Text color={theme.secondaryText}> [{cmd.scope}]</Text>}
                  </Box>
                ))}
              </Box>
            </>
          ...
          {hasCustomCommands() || customCommands.length > 0 ? (
            <Box marginTop={1}>
              <Text color={theme.secondaryText}>Custom commands loaded from:</Text>
              ...
              <Text color={theme.secondaryText}>Use /refresh-commands to reload after changes</Text>
            </Box>
          ) : (
            <Box marginTop={1}>
              <Text color={theme.secondaryText}>Create custom commands by adding .md files to:</Text>
              <Text color={theme.secondaryText}>
                • {getCustomCommandDirectories().userClaude} (Claude `.claude` user scope)
              </Text>
              <Text color={theme.secondaryText}>
              ...
              <Text color={theme.secondaryText}>Use /refresh-commands to reload after creation</Text>
            </Box>
          )}
        </Box>
      )}
