...
export function SelectOption({isFocused, isSelected, smallPointer, children, ...props}: SelectOptionProps) {
  ...
  const styles = {
    ...
    label: ({isFocused, isSelected}: {isFocused: boolean; isSelected: boolean}) => ({
      color: isSelected ? appTheme.success : isFocused ? appTheme.kode : appTheme.text,
      bold: isSelected
    }),
    selectedIndicator: () => ({
  ...
  return (
    <Box {...styles.option({isFocused})}>
      {isFocused && (
        <Text {...styles.focusIndicator()}>{smallPointer ? figures.triangleDownSmall : figures.pointer}</Text>
      )}
