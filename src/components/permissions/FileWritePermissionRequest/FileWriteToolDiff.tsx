...
export function FileWriteToolDiff({file_path, content, verbose, width}: Props): React.ReactNode {
  ...
  return (
    <Box borderColor={getTheme().secondaryBorder} borderStyle="round" flexDirection="column" paddingX={1}>
      <Box paddingBottom={1}>
        <Text bold>{verbose ? file_path : relative(getCwd(), file_path)}</Text>
      </Box>
      {hunks ? (
        intersperse(
          hunks.map(_ => <StructuredDiff key={_.newStart} patch={_} dim={false} width={width} />),
          i => (
        ...
        <HighlightedCode code={content || '(No content)'} language={extname(file_path).slice(1)} />
      )}
    </Box>
  )
}
