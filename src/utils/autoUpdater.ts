...
export async function getLatestVersion(): Promise<string | null> {
  ...
  try {
    ...
    const result = await execFileNoThrow('npm', ['view', MACRO.PACKAGE_URL, 'version'], abortController.signal)
    if (result.code === 0) {
  ...
  try {
    ...
    const res = await fetch(`https://registry.npmjs.org/${encodeURIComponent(MACRO.PACKAGE_URL)}`, {
      method: 'GET',
      headers: {
...
export async function getUpdateCommandSuggestions(): Promise<string[]> {
  return [`bun add -g ${MACRO.PACKAGE_URL}@latest`, `npm install -g ${MACRO.PACKAGE_URL}@latest`]
}
