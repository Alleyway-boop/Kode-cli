...
export type AutoUpdaterStatus = 'disabled' | 'enabled' | 'no_permissions' | 'not_configured'

export function isAutoUpdaterStatus(value: string): value is AutoUpdaterStatus {
  return ['disabled', 'enabled', 'no_permissions', 'not_configured'].includes(value as AutoUpdaterStatus)
}

export type NotificationChannel = 'iterm2' | 'terminal_bell' | 'iterm2_with_bell' | 'notifications_disabled'
...
function saveConfig<A extends object>(file: string, config: A, defaultConfig: A): void {
  ...
  const filteredConfig = Object.fromEntries(
    Object.entries(config).filter(
      ([key, value]) => JSON.stringify(value) !== JSON.stringify(defaultConfig[key as keyof A])
    )
  )
  try {
...
export function enableConfigs(): void {
  ...
  getConfig(GLOBAL_CLAUDE_FILE, DEFAULT_GLOBAL_CONFIG, true /* throw on invalid */)
}
...
export function addMcprcServerForTesting(name: string, server: McpServerConfig): void {
  if (process.env.NODE_ENV === 'test') {
...
export function getConfigForCLI(key: string, global: boolean): unknown {
  if (global) {
    if (!isGlobalConfigKey(key)) {
      console.error(`Error: '${key}' is not a valid config key. Valid keys are: ${GLOBAL_CONFIG_KEYS.join(', ')}`)
      process.exit(1)
    }
    return getGlobalConfig()[key]
  } else {
    if (!isProjectConfigKey(key)) {
      console.error(`Error: '${key}' is not a valid config key. Valid keys are: ${PROJECT_CONFIG_KEYS.join(', ')}`)
      process.exit(1)
    }
...
export function setConfigForCLI(key: string, value: unknown, global: boolean): void {
  if (global) {
    if (!isGlobalConfigKey(key)) {
      console.error(`Error: Cannot set '${key}'. Only these keys can be modified: ${GLOBAL_CONFIG_KEYS.join(', ')}`)
      process.exit(1)
    }
...
export function deleteConfigForCLI(key: string, global: boolean): void {
  if (global) {
    if (!isGlobalConfigKey(key)) {
      console.error(`Error: Cannot delete '${key}'. Only these keys can be modified: ${GLOBAL_CONFIG_KEYS.join(', ')}`)
      process.exit(1)
    }
    const currentConfig = getGlobalConfig()
...
function migrateModelProfilesRemoveId(config: GlobalConfig): GlobalConfig {
  ...
  if ((config as any).defaultModelId) {
    defaultModelName = idToModelNameMap.get((config as any).defaultModelId) || (config as any).defaultModelId
...
export function setModelPointer(pointer: ModelPointerType, modelName: string): void {
  const config = getGlobalConfig()
  const updatedConfig = {
    ...config,
    modelPointers: {
