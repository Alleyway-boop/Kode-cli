...
const SAFE_COMMANDS = new Set(['git status', 'git diff', 'git log', 'git branch', 'pwd', 'tree', 'date', 'which'])
...
export const bashToolHasPermission = async (
  ...
  const commandSubcommandPrefix = await getCommandSubcommandPrefixFn(command, context.abortController.signal)
  ...
  if (subCommands.length < 2) {
    if (bashToolCommandHasPermission(tool, command, commandSubcommandPrefix.commandPrefix, allowedTools)) {
      return {result: true}
    } else {
      return {
        result: false,
...
export async function savePermission(tool: Tool, input: {[k: string]: unknown}, prefix: string | null): Promise<void> {
...
function getPermissionKey(tool: Tool, input: {[k: string]: unknown}, prefix: string | null): string {
  switch (tool) {
    case BashTool:
      if (prefix) {
